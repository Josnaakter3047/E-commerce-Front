...
import { SaleItemService } from '../application-services/sale-item.service';

@Component({
  selector: 'app-sale-print-pdf',
  templateUrl: './sale-print-pdf.component.html',
  styleUrls: ['./sale-print-pdf.component.css']
})
export class SalePrintPdfComponent implements OnInit {
  baseUrl: string='';
  menuId:any;
  salesReport:any;
  vatLabel:any;
  ...
  ngOnInit(): void {
    ...
    this.GetcompanyById();
    // this.columns = [
    //   { field: 'sl', header: '#', show: true, align: 'left' },

    //   { field: 'name', header: this.salesReport?.productName ?? 'Product Name', show: this.salesReport?.isProductName },

    //   { field: 'imei', header: 'IMEI', show: this.salesReport?.imeiByColumn },

    //   { field: 'warranty', header: 'Warranty', show: this.salesReport?.imeiByColumn },

    //   { field: 'unit', header: this.salesReport?.unitName ?? 'Unit', show: this.salesReport?.isUnit },

    //   { field: 'price', header: this.salesReport?.priceName ?? 'Price', show: true }, // ✅ always true

    //   { field: 'qty', header: this.salesReport?.quantity ?? 'Quantity', show: this.salesReport?.isQuantity },

    //   { field: 'discountRate', header: 'Discount Rate', show: this.salesReport?.isDiscountRate },

    //   { field: 'taxRate', header: this.salesReport?.taxRate ?? 'Vat', show: this.salesReport?.isTaxRate },

    //   { field: 'discountAmount', header: 'Dis. Amt', show: true },

    //   { field: 'taxAmount', header: 'Vat Amt', show: true },

    //   { field: 'total', header: this.salesReport?.lineTotal ?? 'Total', show: true } // ✅ always true
    // ];
  }
  ...
  if (this.salesReport?.isProductName) count++;
  if (this.salesReport?.imeiByColumn) count += 2;
  if (this.salesReport?.isUnit) count++;
  if (this.salesReport?.isPrice) count++;
  if (this.salesReport?.isQuantity) count++;
  if (this.salesReport?.isDiscountRate) count++;
  if (this.salesReport?.isDiscountAmount) count++;
  if (this.salesReport?.isTaxRate) count++;
  if (this.salesReport?.isTaxAmount) count++;

  //count += 2; // discountAmount + taxAmount
  count += 1; // total column
