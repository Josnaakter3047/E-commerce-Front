...
@Component({
  selector: 'app-application-user-role-list',
  templateUrl: './application-user-role-list.component.html',
  styleUrls: ['./application-user-role-list.component.css']
})
export class ApplicationUserRoleListComponent implements OnInit {
  list: any;
  loading = true;
  totalRecords = 0;
  noContentLoop = 0;
  ...
  GetAll(): void {
    ...
    if(token){
       this.loading = true;
      this._service.GetAllRoleByCompanyId(token.companyId).subscribe((response) => {
  ...
  onCreate() {
    ...
    this._router.navigate(['/add-application-role',this.menuId]);
    //this._service.displayModal = true;
  }

 onEdit(row: any) {
  this._service.Populate(row);
  this.GetAllRoleFunctionBySp(row.id, row.companyId);
  this._router.navigate(['/add-application-role',this.menuId]);
  //this._service.displayModal = true;
}
GetAllRoleFunctionBySp(roleId:any, companyId){
  this._roleFunctionService.GetAllFunctionBySp(roleId, companyId).subscribe((fun)=>{
    if(fun.statusCode === 200){
    this._service.roleFunctionList = fun.value;
    ...
    accept: () => {
      this._service.DeleteRole(id).subscribe
        (
          ...
          error => {
            this._sharedService.showWarn("Data Exist in Role Permission");
          }
        );
    }
  });
