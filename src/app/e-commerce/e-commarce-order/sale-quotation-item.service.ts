...
import { SalesQuotationItemModel } from './sale-quotation-item';

@Injectable({
  providedIn: 'root'
})
export class SalesQuotationItemService {
  ...
  controller = "/api/SaleQuotationItem/";
  ...
  isDeletableUpdateUrl: string =  this.controller + 'isDeletableUpdate';
  ...
  Populate(model:SalesQuotationItemModel){
  ...
  Add(model:SalesQuotationItemModel){
    const data:SalesQuotationItemModel = {
      saleId:model.saleId,
      productDetailId:model.productDetailId,
      unitId:model.unitId,
      quantity:model.quantity,
  ...
  Update(model:SalesQuotationItemModel){
  ...
  DeletableUpdate(model:any){
    return this._HttpClient.put<any>(`${this.baseUrl}`+this.isDeletableUpdateUrl, model);
  }
  Delete(id:any){
    return this._HttpClient.delete<any>(`${this.baseUrl}`+this.deleteUrl + id);
  }
  updateSaleItems(model:SalesQuotationItemModel[]){
