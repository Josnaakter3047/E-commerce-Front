...
@Component({
  ...
  template: `
     <ng-container>
      <div style="margin-top:5px;" *ngIf="item.items.length > 0">
        <span class="layout-menuitem-text">{{item.name}}</span>
      </div>
      <a (click)="itemClick($event)"
         *ngIf="(item.items)" (keydown.enter)="itemClick($event)"
         [attr.tabindex]="0" (mouseenter)="onMouseEnter()" pRipple
         [pTooltip]="item.name" [tooltipDisabled]="!(root && app.isSlim() && !app.isMobile())">
      ...
      <!-- <a (click)="itemClick($event)"
         *ngIf="(!item.items)"
         ...
         [pTooltip]="item.name" [tooltipDisabled]="active || !(root && app.isSlim() && !app.isMobile())">
        <i [ngClass]="item.icon" class="layout-menuitem-icon"></i>
        <span class="layout-menuitem-text">{{item.name}}</span>
